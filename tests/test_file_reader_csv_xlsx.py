...
@patch("csv.reader")
def test_read_csv_valid_data(mock_csv_reader, mock_file):
    """1. Тест чтения CSV-файла с валидными данными."""
    read_csv("test.csv")
    mock_file.assert_called_once_with(os.path.join(DATA_DIR, "test.csv"), encoding="utf-8")
    mock_csv_reader.assert_called_once_with(mock_file(), delimiter=';')


@patch("builtins.open", new_callable=mock_open, read_data="")
@patch("csv.reader")
def test_read_csv_empty_file(mock_csv_reader, mock_file):
    """2. Тест чтения пустого CSV-файла."""
    read_csv("empty.csv")
    mock_file.assert_called_once_with(os.path.join(DATA_DIR, "empty.csv"), encoding="utf-8")
    mock_csv_reader.assert_called_once_with(mock_file(), delimiter=';')


@patch("builtins.open", side_effect=FileNotFoundError)
def test_read_csv_file_not_found(mock_file):
    """3. Тест обработки отсутствия CSV-файла."""
    with unittest.TestCase().assertRaises(FileNotFoundError):
        read_csv("missing.csv")
    mock_file.assert_called_once_with(os.path.join(DATA_DIR, "missing.csv"), encoding="utf-8")
